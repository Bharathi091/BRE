...
sap.ui.define([
	...
	var services = Object.extend("dbedit.Services.MatterServices", {
		...
		getmattersSetfilter: function(BillEditModel, filter, that) {
			...
			var masterServiceUrl = that.getOwnerComponent().getModel().sServiceUrl;
			jQuery.ajax({
		...
		getOutPutTypesets: function(BillEditModel, billModel, that) {
			...
			var beditModel = new sap.ui.model.odata.v2.ODataModel({
				serviceUrl: that.getOwnerComponent().getModel().sServiceUrl
			}, true);
		...
		getfinalmassbilltypes: function(BillEditModel, billModel, that) {
			debugger;
			var deferred = $.Deferred();
			var aVbeln = [];
			var serviceUrls = BillEditModel.getProperty("/Inputs/services");
			...
			var FBMassOutputTypeSet = serviceUrls.FBMassOutputTypeSet;
			var PrintBillsfilter = [new sap.ui.model.Filter({
				path: "Kappl",
				operator: sap.ui.model.FilterOperator.EQ,
				value1: "V3"
			})];
			for (var i = 0; i < billModel.length; i++) {
			aVbeln =	new sap.ui.model.Filter({
							path: "Vbeln",
							operator: sap.ui.model.FilterOperator.EQ,
							value1: billModel[i].Vbeln
						});
			}
            PrintBillsfilter.push(aVbeln);
			beditModel.read(FBMassOutputTypeSet, {
				filters: PrintBillsfilter,
				success: function(oData) {
					deferred.resolve(oData);
				},
